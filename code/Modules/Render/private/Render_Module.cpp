...
namespace Render
{
	...
	Renderer* RenderModule::GetRenderer(GLFWwindow* aWindow) const
	{
		auto it = std::find_if(mySwapChains.begin(), mySwapChains.end(), [aWindow](SwapChain* aSwapChain) { return aSwapChain->GetWindowHandle() == aWindow; });
		if (it != mySwapChains.end())
		{
	...
	VkPhysicalDevice RenderModule::GetPhysicalDevice() const
	{
		return myDevice->myPhysicalDevice;
	}

	VkDevice RenderModule::GetDevice() const
	{
		return myDevice->myLogicalDevice;
	}

	VmaAllocator RenderModule::GetAllocator() const
	{
		return myDevice->myVmaAllocator;
	}

	VkQueue RenderModule::GetGraphicsQueue() const
	{
		return myDevice->myGraphicsQueue;
	}

	VkCommandPool RenderModule::GetGraphicsCommandPool() const
	{
		return myDevice->myGraphicsCommandPool;
	}


	VkDescriptorSetLayout RenderModule::GetDescriptorSetLayout(ShaderHelpers::BindType aType)
	{
	...
	VkDescriptorSet RenderModule::GetDescriptorSet(ShaderHelpers::BindType aType, const ShaderHelpers::DescriptorInfo& someDescriptorInfo)
	{
	...
	void RenderModule::CreateVkInstance()
	{
	...
	void RenderModule::CreateDevice()
	{
		uint deviceCount = 0;
	...
	void RenderModule::SetupDefaultData()
	{
	...
	void RenderModule::DestroyDefaultData()
	{
	...
	void RenderModule::SetupTexture(VulkanImage& anImage, uint8* aColor)
	{
		anImage.Create(1, 1,
	...
	void RenderModule::UpdateMaxInFlightFramesCount()
	{
	...
	void RenderModule::RecycleDescriptorSets()
	{
		for (DescriptorContainer& container : myDescriptorContainers)
