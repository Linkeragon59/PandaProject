...
void vkglTF::Texture::destroy()
{
	if (device)
	{
		vkDestroyImageView(device->myLogicalDevice, view, nullptr);
		vmaDestroyImage(device->myVmaAllocator, image, deviceMemory);
		vkDestroySampler(device->myLogicalDevice, sampler, nullptr);
	}
}
...
void vkglTF::Material::createDescriptorSet(VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout, uint32_t descriptorBindingFlags)
{
	...
	vkUpdateDescriptorSets(device->myLogicalDevice, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
}
...
vkglTF::Model::~Model()
{
	...
	if (descriptorSetLayoutUbo != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(device->myLogicalDevice, descriptorSetLayoutUbo, nullptr);
		descriptorSetLayoutUbo = VK_NULL_HANDLE;
	}
	if (descriptorSetLayoutImage != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(device->myLogicalDevice, descriptorSetLayoutImage, nullptr);
		descriptorSetLayoutImage = VK_NULL_HANDLE;
	}
	vkDestroyDescriptorPool(device->myLogicalDevice, descriptorPool, nullptr);
