...
namespace glTF
{
	void Animation::Load(Model* aContainer, const tinygltf::Model& aModel, uint32_t anAnimationIndex)
	{
		...
		for (uint32_t i = 0; i < (uint32_t)gltfAnimation.samplers.size(); ++i)
		{
		...
		for (uint32_t i = 0; i < (uint32_t)gltfAnimation.channels.size(); ++i)
		{
